...
BEGIN
    ...
    IF p_personal < 0 THEN
        DBMS_OUTPUT.PUT_LINE('EL personal debe ser positivo'); -- Imprimir mensaje de error
        RETURN; -- Salir del procedimiento
    END IF;
...
BEGIN
    ...
    SELECT COUNT(*) INTO repetido FROM PUESTOS WHERE NOMBRE=p_nombre;

    IF repetido > 0 THEN
        DBMS_OUTPUT.PUT_LINE('PUESTO YA REGISTRADO.'); -- Imprimir mensaje de error
        RETURN; -- Salir del procedimiento
    END IF;
...
BEGIN
    ...
    SELECT COUNT(*) INTO v_duplicado FROM EMPLEADOS WHERE DPI = p_dpi;
    IF v_duplicado>0 THEN
        DBMS_OUTPUT.PUT_LINE('DPI DUPLICADO'); -- Imprimir mensaje de error
        RETURN; -- Salir del procedimiento
    END IF;
    ...
    VALUES (p_nombre,p_apellido,TO_DATE(p_fecha, 'YYYY-MM-DD'),p_correo,p_telefono,p_direccion,p_dpi,p_puesto,TO_DATE(p_fecha_inicio, 'YYYY-MM-DD') ,p_id_restaurante);
...
BEGIN
    ...
    IF LENGTH(p_nit) > 0 THEN
        ...
        VALUES (p_dpi,p_nombre,p_apellido,TO_DATE(p_fecha, 'YYYY-MM-DD'),p_correo,p_telefono,p_nit);
    ELSE
        INSERT INTO CLIENTES  
        VALUES (p_dpi,p_nombre,p_apellido,TO_DATE(p_fecha, 'YYYY-MM-DD'),p_correo,p_telefono,'CF');
    END IF;
...
BEGIN
    ...
    SELECT COUNT(*) INTO p_dir_cliente FROM DIRECCIONES WHERE ID = p_direccion AND p_dpi=DPI_CLIENTE;

    IF p_dir_cliente=0 THEN
        DBMS_OUTPUT.PUT_LINE('ERROR DIRECCION PERTENECE A OTRO CLIENTE'); -- Imprimir mensaje de error
        RETURN; -- Salir del procedimiento
    END IF;
...
AS
    ...
    CURSOR c_ordenes_m IS
        SELECT o.ID, o.ESTADO, o.Inicio AS FECHA, o.DPI_CLIENTE AS DPI_CLIENTE, d.Direccion, o.Canal
        FROM ORDENES o 
        INNER JOIN DIRECCIONES d ON o.Id_Direccion = d.Id 
        WHERE o.ESTADO = s_estado;
    r_ordenes c_ordenes%ROWTYPE;
   	r_ordenes_m c_ordenes_m%ROWTYPE;
BEGIN
    IF p_estado = 1 THEN
        s_estado:='INICIADA';
    ...
    IF p_estado = -1 THEN
        OPEN c_ordenes_m;
            LOOP
                FETCH c_ordenes_m INTO r_ordenes_m;
                EXIT WHEN c_ordenes_m%NOTFOUND;
                DBMS_OUTPUT.PUT_LINE('ID: ' || r_ordenes_m.ID);
                DBMS_OUTPUT.PUT_LINE('ESTADO: ' || r_ordenes_m.ESTADO);
                DBMS_OUTPUT.PUT_LINE('FECHA: ' || r_ordenes_m.FECHA);
                DBMS_OUTPUT.PUT_LINE('DPI CLIENTE: ' || r_ordenes_m.DPI_CLIENTE);
                DBMS_OUTPUT.PUT_LINE('DIRECCION: ' || r_ordenes_m.DIRECCION);
                DBMS_OUTPUT.PUT_LINE('CANAL: ' || r_ordenes_m.CANAL);
                DBMS_OUTPUT.PUT_LINE('------------------------');
...
BEGIN
    fecha_creada := TO_CHAR(p_año) || '-' || TO_CHAR(p_mes) || '-' || TO_CHAR(p_dia);
    OPEN c_facturas;
    LOOP
